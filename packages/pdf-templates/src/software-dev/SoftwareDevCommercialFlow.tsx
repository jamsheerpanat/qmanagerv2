        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "7.5px", border: "1px solid #e2e8f0", borderRadius: "5px", overflow: "hidden", marginBottom: "12px" }}>
          ...
          <tbody>
            {(items || []).map((item: any, idx: number) => {
              if (item.itemType === "SECTION_HEADING") {
                return (<tr key={idx} style={{ breakInside: "avoid", pageBreakInside: "avoid" }}><td colSpan={8} style={{ padding: "5px 8px", fontWeight: "700", color: accent, fontSize: "7px", letterSpacing: "0.4px", background: accentLight, borderLeft: `2.5px solid ${accent}`, borderBottom: `1px solid ${accentHighlight}44` }}>{item.sectionTitle}</td></tr>);
              }
              serial++;
