...
export const ITInfraQuotationPage = ({ items, subtotal, discount, tax, grandTotal, currency = "KWD" }: any) => {
  const fmt = (n: any) => Number(n || 0).toLocaleString("en-US", { minimumFractionDigits: 3, maximumFractionDigits: 3 });

  // Color theme - deep navy/sky for IT infra
  const accent = "#0369a1";
  const accentLight = "#e0f2fe";
  const accentGrad = "linear-gradient(135deg,#0c1a2e,#0c2d50)";
  const accentHighlight = "#38bdf8";
  ...
  return (
    <div className="pdf-page" style={{ fontFamily: "'Inter', 'Montserrat', sans-serif", background: "white" }}>
      <div style={{ padding: "14mm 16mm", height: "297mm", display: "flex", flexDirection: "column" }}>
        <ITHeader pageTitle="Commercial Quotation" />
        ...
        <div style={{ width: "60px", height: "3px", background: `linear-gradient(90deg,${accent},${accentHighlight})`, borderRadius: "2px", marginBottom: "16px" }} />
              ...
              {(items || []).map((item: any, idx: number) => {
                ...
                return (
                  <tr key={idx} style={{ background: isEven ? "white" : "#f8fafc", borderBottom: "1px solid #f1f5f9" }}>
                    ...
                    <td style={{ padding: "9px 8px", textAlign: "center", color: "#64748b", fontSize: "7.5px" }}>{item.brand || "—"}</td>
