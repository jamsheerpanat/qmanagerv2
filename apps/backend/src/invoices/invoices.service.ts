...
import { InvoiceStatus, PaymentStatus, InvoiceType, DiscountType } from '@prisma/client';
...
export class InvoicesService {
  ...
  async createFromQuotation(quotationId: string, type: InvoiceType, userId?: string) {
    ...
    if (!['APPROVED', 'ACCEPTED', 'SENT_TO_CUSTOMER'].includes(q.status)) {
      throw new BadRequestException('Quotation must be approved or accepted to convert to invoice');
    }
  ...
  async update(id: string, dto: UpdateInvoiceDto) {
    ...
    if (inv.invoiceStatus !== InvoiceStatus.DRAFT) {
      throw new BadRequestException('Only DRAFT invoices can be freely modified');
    }
  ...
  async replaceItems(id: string, itemsDto: InvoiceItemDto[]) {
    const inv = await this.prisma.invoice.findUnique({ where: { id } });
    if (!inv || inv.invoiceStatus !== InvoiceStatus.DRAFT) throw new BadRequestException('Invoice not draft');
    ...
    const itemsData = itemsDto.map((item, index) => {
      ...
      if (item.discountValue) {
        if (item.discountType === DiscountType.PERCENTAGE) discountAmount = (qty * price) * (item.discountValue / 100);
  ...
  private async recalculateTotals(id: string) {
    ...
    if (inv.discountValue) {
      if (inv.discountType === DiscountType.PERCENTAGE) qDiscountAmount = subtotal * (inv.discountValue / 100);
  ...
  async issue(id: string) {
    ...
    if (!inv || inv.invoiceStatus !== InvoiceStatus.DRAFT) throw new BadRequestException('Not in DRAFT state');

    const prefix = inv.company.invoicePrefix || 'INV';
    const year = new Date().getFullYear();
    const count = await this.prisma.invoice.count({
      where: { companyId: inv.companyId, invoiceDate: { gte: new Date(`${year}-01-01`), lte: new Date(`${year}-12-31`) }, invoiceStatus: { not: InvoiceStatus.DRAFT } }
    });
    ...
    await this.prisma.invoice.update({
      where: { id },
      data: { invoiceNumber, invoiceStatus: InvoiceStatus.ISSUED, invoiceDate: new Date() }
    });
  ...
  async recordPayment(id: string, dto: RecordPaymentDto, userId?: string) {
    ...
    if (!inv || inv.invoiceStatus === InvoiceStatus.DRAFT || inv.invoiceStatus === InvoiceStatus.CANCELLED) {
      throw new BadRequestException('Cannot pay Draft or Cancelled invoice');
    }
    
    if (dto.amount <= 0 || dto.amount > inv.balanceAmount + 0.01) {
      throw new BadRequestException(`Payment amount ${dto.amount} is invalid or exceeds balance ${inv.balanceAmount}`);
    }
    ...
    if (paymentStatus === PaymentStatus.PAID) invoiceStatus = InvoiceStatus.PAID;
    else if (paymentStatus === PaymentStatus.PARTIALLY_PAID) invoiceStatus = InvoiceStatus.PARTIALLY_PAID;
