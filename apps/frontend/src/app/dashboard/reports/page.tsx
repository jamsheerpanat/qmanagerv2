...
export default function ReportsPage() {
  ...
  const handleExport = () => {
    ...
    window.open(`${process.env.NEXT_PUBLIC_API_URL}/reports/${reportType}?${params.toString()}`, "_blank");
  };
        ...
        <CardContent className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            <div>
              ...
              <select className="w-full border rounded p-2" value={reportType} onChange={e => { setReportType(e.target.value); setData([]); }}>
          ...
          <div className="mt-6 flex justify-end">
            <Button onClick={handleGenerate} className="bg-blue-600 hover:bg-blue-700" disabled={loading}>
              <Search className="w-4 h-4 mr-2" /> {loading ? 'Generating...' : 'Generate Report'}
            </Button>
          </div>
        </CardContent>
      </Card>
            ...
            <thead>
              <tr className="bg-gray-50 border-b text-gray-500 text-sm">
                {data.length > 0 ? Object.keys(data[0]).map(key => (
                  <th key={key} className="p-4 font-medium uppercase tracking-wider">{key}</th>
                )) : (
                  <th className="p-4 font-medium uppercase tracking-wider">Data Preview</th>
                )}
              </tr>
            </thead>
            <tbody>
              {data.length > 0 ? (
                ...
                <tr>
                  <td className="p-8 text-center text-gray-400" colSpan={10}>
                    {loading ? 'Fetching data...' : 'Select filters and generate report to see preview here.'}
                  </td>
                </tr>
              )}
            </tbody>
